...
----
  ...
  文字列は、`"`で括って記述します。`"`から`"`までが文字列です。

    "abc"

  文字列内では`\”`と記述する事で`"`を文字列内に含める事が出来ます。
  `\\`は`\`、`\'`は`'`、`\t`はタブ、`\r`はCR、`\n`はLF、 `\b`はバックスペース、`\ddd`はアスキーコードの10進数表現を表します。例えば`\097`はアスキーコード97の文字`a`です。
...
## 22. モジュール
  ...
  このモジュールはaという値とincという関数を持ったモジュールです。
  以下のように`モジュール名.変数名`あるいは`モジュール名.関数名`と書くことで各モジュールの要素にアクセス出来ます。
